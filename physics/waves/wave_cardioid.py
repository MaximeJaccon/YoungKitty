...
# Define cardioid mask
theta = np.arctan2(Y, X_shifted)
r = np.sqrt(X_shifted**2 + Y**2)
a = 300
r_cardioid = 2 * a * (1 - np.cos(theta))
mask = r <= r_cardioid
...
ax.set_title("2D Wave Propagation in a Cardioid Domain")
ax.axis('off')
...
ani.save('wave_propagation.mp4', writer='ffmpeg', fps=30, dpi=300)
